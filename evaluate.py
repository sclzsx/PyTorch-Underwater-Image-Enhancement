...
def print_np_info(name, m):
    print(name, type(m), m.shape, m.dtype, 'min:{}, max:{}, mean:{}'.format(np.min(m), np.max(m), np.mean(m)))
...
def main():
    result_path = '3_datasets/UIEBD/raw-890'
    reference_path = '3_datasets/UIEBD/reference-890'
    ...
    for imgdir in result_dirs:
        if '.png' in imgdir:
            ...
            print_np_info('corrected', corrected)
            ...
            print_np_info('reference', reference)
